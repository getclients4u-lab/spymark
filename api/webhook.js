// Vercel serverless webhook for Stripe payment events (spymark)
...
const MAIL_FROM = process.env.SPYMARK_MAIL_FROM || 'gentledesk632@agentmail.to';
const AGENTMAIL_KEY = process.env.AGENTMAIL_API_KEY || '';
const BUYERS_FILE = 'buyers.json';
const USERS_FILE = 'users.json';
const ACCESS_PEPPER = process.env.ACCESS_PEPPER || 'spymark-pepper-qwonahu5';
...
async function sendEmail(buyer, code) {
  ...
  const body =
`Welcome to SpyMark Defense™ — the creator's defense against invisible AI tracking marks ✅

Hi ${buyer.name || 'there'},

Thank you! Your order is confirmed.

YOUR PERSONAL ACCESS CODE: ${code || 'SP-XXXXX-XXXXX'}

OPEN YOUR DOWNLOADS:
→ https://spymark.vercel.app/download.html
(enter your email + access code above)

WHAT'S INSIDE (8 deliverables):
• 1. The SpyMark Protection System™ (core guide + DSSB model + 7-day sprint)
• 2. The Tracking Audit Sheet™ + Origin Log™ (risk score + proof of authorship)
• 3. The Strip & Seal Toolkit™ (disclosure blocks, badge spec, cleanup commands)
• 4. The Trust Premium Pricing Sheet™ (tiers, levers, premium script)
• 5. The Pre-Publish Checklist™ (the 8-minute gate)
• 6. The SpyMark Tracker™ (weekly scorecard + 30-day Trust Score)
• 7. The Provenance Scripts™ (answers to the 12 hardest questions)
• 8. The SpyMark Playbook™ (30-day roadmap + Starter Block)

START TONIGHT (30-minute Starter Block):
1. Run exiftool -a -G1 -s on your last published file — see what it says about you.
2. Strip it: exiftool -all= file — then re-export from your editor.
3. Log your first Origin Record entry (deliverable 2). Title, date, tools, human work.

Then run the Pre-Publish Checklist on everything you publish from now on.

Detect. Strip. Seal. Broadcast. Charge for the difference.

Remember: strip tracking, disclose tools — always. They are opposite acts.
...
const res = await fetch(`https://api.agentmail.to/v0/inboxes/${MAIL_FROM}/messages/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${AGENTMAIL_KEY}` },
    body: JSON.stringify({ to: [buyer.email], subject: "Your SpyMark Defense™ access ✅", text: body }),
  });
  return res.ok;
}
